...
**Branch base:** `develop`
**Data:** 2026-07-31
**Design de referência:** `docs/renovacao-automatica-assinatura.puml` e `docs/renovacao-automatica-retry-dlq.puml`
...
## 0. Pré-requisito: o 0.2.0 precisa existir (parcialmente)

A renovação reusa a infraestrutura de webhook e o client do gateway construídos
no 0.2.0. Hoje, no `develop`, **o webhook do Pagamento (Track E do 0.2.0) ainda
não existe**. Consequência:

- BE-7, BE-8, BE-11 e as tracks que delas dependem rodam **em paralelo ao
  fechamento do 0.2.0**, porque codificam contra contratos, não contra a
  implementação do webhook de ativação.
- BE-13 (webhook de renovação) **fica bloqueada** até o webhook de ativação do
  0.2.0 ser merged em `develop`. Ele herda o endpoint `POST /webhooks/payments`
  e o padrão publish-then-ACK.

Não há bloqueio para começar agora. Há bloqueio pontual numa track.
...
| ID | Decisão | Racional |
|----|---------|----------|
| D-R1 | **Outbox roteia por `eventType`** (mapa evento→tópico), não mais hardcoded em `assinatura-solicitada` | O `OutboxPublisher` atual publica **todo** evento no tópico `assinatura-solicitada` (`OutboxPublisher.java:50`). Para publicar `RenovacaoSolicitada` no tópico certo, o roteamento vira função do `eventType`. Fundação da BE-7. |
...
| D-R7 | **`Assinatura` ganha campos de ciclo** e novos status | Campos: `inicioCiclo`, `fimCiclo`, `proximaRenovacaoEm`, `renovacaoAutomatica`. Status novos: `EM_RENOVACAO`, `SUSPENSA`, `CANCELADA`. Método `renovar()` distinto de `ativar()` (que só aceita partir de `AGUARDANDO_PAGAMENTO`). A ativação inicial define o primeiro ciclo; a renovação rola o ciclo para frente. |
...
- **Coordenação de migrations (importante):** a numeração precisa ser combinada para não colidir entre tracks do mesmo serviço.
  - Assinatura está em `V6`. BE-8 cria `V7` (tabela `renovacao` + colunas de ciclo em `assinatura`). BE-10 cria `V8` (tabela de dedup `renovacao_evento_processado`). BE-7 não cria migration.
  - Pagamento está em `V1`. BE-11 cria `V2` (`pagamento_renovacao` + `tentativa_cobranca`). BE-13 cria `V3` (dedup de `eventId` de webhook de renovação).
...
- **Gate 0 — Contratos de renovação travados.** Criar `docs/contrato-eventos-renovacao.puml` espelhando a tabela do §2 (mesmo formato do `contrato-eventos-kafka.puml`). **Desbloqueia todas as tracks BE-7…BE-13.** É um doc, não código; pode ser feito imediatamente.
- **Gate 0.2.0 — Webhook de ativação merged.** Desbloqueia **apenas** BE-13. As demais tracks já rodam contra o `develop` atual.

---

## 5. Tracks (executáveis independentemente)

### BE-7 — Outbox roteia por `eventType` (Assinatura) — fundação
- **Serviço:** Assinatura
- **Depende de:** Gate 0
- **Risco:** toca o `OutboxPublisher` que toda publicação usa. Mudança isolada e testável.
- **Consome:** design `docs/outbox-assinatura-solicitada.puml` e `docs/outbox-modelo-dados.puml`.
- **Definition of Done:**
  - O `OutboxPublisher` resolve o tópico por `eventType` (mapa configurável `eventType → tópico`), em vez do tópico fixo `assinatura-solicitada` (`OutboxPublisher.java:50`).
...
- **Paralelizável com:** BE-8, BE-11.

### BE-8 — Domínio de ciclo + agregado `Renovacao` + migration (Assinatura) — fundação
- **Serviço:** Assinatura
- **Depende de:** Gate 0
- **Consome:** design `docs/renovacao-automatica-assinatura.puml` (linhas 26-72, 165-195).
- **Definition of Done:**
...
- **Serviço:** Assinatura
- **Depende de:** BE-8, BE-7 (para o evento ser publicado)
- **Consome:** design `docs/renovacao-automatica-assinatura.puml` (linhas 23-72) e `docs/renovacao-automatica-retry-dlq.puml` (linhas 34-59).
- **Definition of Done:**
...
- **Paralelizável com:** BE-10, BE-11, BE-12 (streams distintos). Contenção de domínio com BE-10 → ver §6.

### BE-10 — Consumer de resultado da renovação: renova ou suspende (Assinatura)
- **Serviço:** Assinatura
- **Depende de:** BE-8, Gate 0 (contrato de resultado)
- **Consome:** design `docs/renovacao-automatica-assinatura.puml` (linhas 165-195) e `docs/renovacao-automatica-retry-dlq.puml` (linhas 203-224).
- **Definition of Done:**
...
- **Paralelizável com:** BE-9, BE-11, BE-12. Contenção de domínio com BE-9 → ver §6.

### BE-11 — Agregado pagamento-renovação + consumer `RenovacaoSolicitada` (Pagamento) — fundação
- **Serviço:** Pagamento
- **Depende de:** Gate 0
- **Consome:** design `docs/renovacao-automatica-assinatura.puml` (linhas 78-87) e `docs/renovacao-automatica-retry-dlq.puml` (linhas 96-121).
- **Definition of Done:**
  - Migration `V2`: `pagamento_renovacao` (`UNIQUE (renovacao_id)`, idempotência por renovação) e `tentativa_cobranca` (`renovacao_id`, `numero` int, `status`, `proxima_tentativa_em`, `payment_id`).
...
- **Arquivos previstos:** `PagamentoRenovacao`, `StatusPagamentoRenovacao`, `TentativaCobranca`, `StatusTentativa`, repositories, `V2__*.sql`, `RenovacaoSolicitada` (event), `RenovacaoSolicitadaConsumer`, `KafkaTopicsConfig`.
...
### BE-12 — Scheduler de tentativas de cobrança: chama o gateway, publica resultado (Pagamento)
- **Serviço:** Pagamento
- **Depende de:** BE-11
- **Consome:** design `docs/renovacao-automatica-assinatura.puml` (linhas 89-163) e `docs/renovacao-automatica-retry-dlq.puml` (linhas 131-201).
- **Definition of Done:**
...
- **Serviço:** Pagamento
- **Depende de:** BE-11 **e** webhook de ativação do 0.2.0 merged em `develop` (Gate 0.2.0)
- **Consome:** design `docs/renovacao-automatica-assinatura.puml` (linhas 113-163) e `docs/renovacao-automatica-retry-dlq.puml` (linhas 154-201).
- **Definition of Done:**
  - Estende o endpoint `POST /webhooks/payments` (do 0.2.0) para despachar conforme o `externalReference`: se for uma `renovacaoId` conhecida, segue o fluxo de renovação.
  - Valida HMAC, consulta status oficial (`GET /v1/payments/{paymentId}`), normaliza (D4 do 0.2.0), registra `eventId` **após** publish bem-sucedido (publish-then-ACK, mesmo padrão da ativação).
  ...
  - Migration `V3`: dedup de `eventId` de webhook de renovação (ou generalização da tabela do 0.2.0).
- **Arquivos previstos:** `WebhookPagamentoController` (ramo de renovação), entidade/repo de webhooks processados de renovação, `V3__*.sql`, eventos `PagamentoRenovacaoAprovado`/`RenovacaoTentativasEsgotadas`.
...
Gate 0: contratos renovação travados (doc)
   ...
   ├──────── STREAM PAGAMENTO ────────────────────────┐
   │                                                  │
   │   BE-11: agregado pagto-renov + consumer (V2,fnd)│
   │     ├──> BE-12: scheduler tentativas cobrança    │
   │     └──> BE-13: webhook renovação (V3)           │
   │            [bloqueia até webhook 0.2.0 em develop]│
   │                                                  │
   └────────────────────────┬─────────────────────────┘
                            ▼
...
- **Concorrência máxima:** 2 streams. Após as fundações (`BE-8`, `BE-11`), até 4 tracks em paralelo: `BE-9`, `BE-10`, `BE-12`, `BE-13` (esta última, assim que o webhook do 0.2.0 mergear).
...
- **Contenção dentro do Stream Pagamento:** `BE-12` (escreve `TentativaCobranca`) e `BE-13` (atualiza `TentativaCobranca`) compartilham o agregado. Sugerir **merge de BE-12 antes de BE-13**, ou coordenar a migração `V3` (dedup).
- **Long pole:** Stream Assinatura (`BE-8` fundação + `BE-9`/`BE-10`); Stream Pagamento é sequencial (`BE-11` → `BE-12`/`BE-13`) mas cada peça é leve. `BE-13` é a track que mais espera (depende do 0.2.0).
...
2. **Fechar o 0.2.0 antes** (webhook de ativação + `release/0.2.0`), já que o 0.3.0 depende do webhook de ativação.
