...
@WebMvcTest(controllers = OutboxFalhaController.class)
@Import({SecurityConfig.class, OutboxFalhaExceptionHandler.class})
class OutboxFalhaControllerTest {
  ...
  void deveListarEventosEmFalhaParaAdministrador() throws Exception {
    OutboxFalhaItem item =
        new OutboxFalhaItem(
            ...
            3);
    when(listarFalhasOutbox.executar("PagamentoStatusAtualizado", 3600L, 0, 20))
    ...
    mockMvc
        ...
        .andExpect(jsonPath("$.items[0].eventType").value("PagamentoStatusAtualizado"))
        .andExpect(jsonPath("$.items[0].falhouEm").value("2026-08-03T10:00:00Z"))
        .andExpect(jsonPath("$.items[0].ciclosRecuperacao").value(3))
        .andExpect(jsonPath("$.total").value(1));

    verify(listarFalhasOutbox).executar("PagamentoStatusAtualizado", 3600L, 0, 20);
  }
  ...
  void deveRetornarForbiddenParaClienteNaListagem() throws Exception {
    mockMvc
        .perform(get("/outbox/falhas").with(authentication(cliente())))
